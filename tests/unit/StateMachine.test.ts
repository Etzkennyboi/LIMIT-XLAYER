...
describe('StateMachine', () => {
    ...
    test('invalid transition SETTLED -> MONITORING throws', async () => {
        await expect(
            sm.transition('id-2', IntentStatus.SETTLED, IntentStatus.MONITORING)
        ).rejects.toThrow(LimitOrderError);
    });

    test('CONFIRMED can only go to SETTLED', async () => {
        await expect(
            sm.transition('id-3', IntentStatus.CONFIRMED, IntentStatus.CANCELLED)
        ).rejects.toThrow(LimitOrderError);
    });
});
