...
export class RedisService {
    ...
    async setIdempotency(key, intentId, ttlSeconds = 60) {
    ...
    async updateIntentStatus(id, oldStatus, newStatus, updates = {}) {
        const pipeline = this.client.pipeline();
    ...
    // ── Queue Operations ─────────────────────────────────────────
    async getPendingIntents(maxExpiry) {
    ...
    async pushTriggeredIntent(id) {
        await this.client.rpush('lo:queue:triggered', id);
    }
    async popTriggeredIntent(timeoutSec) {
    ...
    async logTransition(transition) {
        await this.client.xadd('lo:audit', '*', 'intentId', transition.intentId, 'from', transition.from, 'to', transition.to, 'timestamp', String(transition.timestamp), 'reason', transition.reason || '', 'signature', transition.signature, 'metadata', JSON.stringify(transition.metadata || {}));
    }
    async getAuditLog(intentId, count = 100) {
        ...
        return entries
            ...
            .map(([, fields]) => {
            const get = (k) => fields[fields.indexOf(k) + 1];
            return {
    ...
    // ── Queries ──────────────────────────────────────────────────
    async countActiveIntents(userId) {
