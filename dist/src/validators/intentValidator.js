...
import { IntentType, SupportedChain } from '../types.js';
import { PriceConfidence } from '../types.js';
import { LimitOrderError, Errors } from '../utils/errors.js';
import { config } from '../config.js';
export function validateIntentParams(params, currentPrice, walletBalance) {
    // 1. Amount
    const amount = new BigNumber(params.amount);
    if (amount.lte(0) || amount.gt(walletBalance)) {
        throw new LimitOrderError(Errors.INVALID_AMOUNT.code, Errors.INVALID_AMOUNT.message, { amount: params.amount, balance: walletBalance });
    }
    // 2. Chain
    ...
    if (currentPrice.confidence === PriceConfidence.LOW || currentPrice.sources < 2) {
        throw new LimitOrderError(Errors.LOW_CONFIDENCE_PRICE.code, Errors.LOW_CONFIDENCE_PRICE.message, { confidence: currentPrice.confidence, sources: currentPrice.sources });
    }
    ...
    if (params.type === IntentType.OCO && params.ocoConfig) {
        ...
        if (slDelta.lt(config.MIN_OCO_STOP_DELTA_BPS)) {
            throw new LimitOrderError(Errors.INVALID_OCO.code, `OCO stop-loss must be at least ${config.MIN_OCO_STOP_DELTA_BPS / 100}% below current price`);
        }
    }
    else {
        // Standard intents: minimum 10% delta
        if (deltaBps.lt(config.MIN_TRIGGER_DELTA_BPS)) {
            throw new LimitOrderError(Errors.INVALID_TRIGGER.code, Errors.INVALID_TRIGGER.message, { deltaBps: deltaBps.toFixed(2), required: String(config.MIN_TRIGGER_DELTA_BPS) });
        }
    }
}
