...
import { ConsensusPrice, PriceConfidence, SupportedChain } from '../types.js';
import { WebSocketManager } from './WebSocketManager.js';
import { config, CHAINLINK_RPCS } from '../config.js';
import { logger } from '../utils/logger.js';

const WEIGHTS = { okx: 0.5, chainlink: 0.3, binance: 0.2 } as const;
...
export class PriceConsensus {
  ...
  async getPrice(chain: SupportedChain, tokenAddress: string): Promise<ConsensusPrice | null> {
    const cacheKey = `${chain}:${tokenAddress.toLowerCase()}`;
    const cached = this.cache.get(cacheKey);
    if (cached && Date.now() < cached.expiresAt) return cached.consensus;
    ...
    const sources: { price: BigNumber; weight: number }[] = [];
    ...
    const confidence: PriceConfidence =
      variance < 0.01 ? PriceConfidence.HIGH :
  ...
  private async getOkxPrice(_chain: SupportedChain, tokenAddress: string): Promise<string | null> {
  ...
  private async getChainlinkPrice(chain: SupportedChain, tokenAddress: string): Promise<string | null> {
    const rpc = CHAINLINK_RPCS[chain];
    if (!rpc) return null;  // gracefully skip — not configured
    try {
  ...
  private async getBinancePrice(tokenAddress: string): Promise<string | null> {
    ...
    try {
      const { data } = await axios.get(
        `${config.BINANCE_REST_URL}/api/v3/ticker/price`,
