...
export class TeeService {
  private readonly isProd = config.NODE_ENV === 'production';

  async signTransition(data: TransitionData): Promise<string> {
    if (this.isProd) {
    ...
    if (!config.DEV_SECRET) {
      throw new LimitOrderError(
        Errors.TEE_UNAVAILABLE.code,
        'DEV_SECRET must be set in non-production environments'
      );
    }
    return crypto
  ...
  async signTransaction(txData: unknown): Promise<{ signedTx: string; signature: string }> {
    if (this.isProd) {
  ...
  async verifySignature(signature: string, data: unknown): Promise<boolean> {
    if (this.isProd) {
  ...
  private async callTeeEndpoint<T>(path: string, body: unknown): Promise<T> {
    if (!config.TEE_ENDPOINT) {
      throw new LimitOrderError(Errors.TEE_UNAVAILABLE.code, 'TEE_ENDPOINT not configured');
    }
    try {
      const { data } = await axios.post<T>(
        `${config.TEE_ENDPOINT}${path}`,
        body,
        { timeout: 5000 }
      );
      return data;
    } catch (err) {
      logger.error('TEE endpoint error', { path, error: (err as Error).message });
