...
import {
  Intent, IntentStatus, IntentType, CreateIntentParams
} from '../types.js';
...
export class IntentEngine {
  ...
  async createIntent(params: CreateIntentParams): Promise<Intent> {
    ...
    if (existing && Date.now() - existing.createdAt < 60_000) {
      throw new LimitOrderError(Errors.DUPLICATE_INTENT.code, Errors.DUPLICATE_INTENT.message,
    ...
    if (activeCount >= config.MAX_ACTIVE_INTENTS_PER_USER) {
      throw new LimitOrderError(Errors.RATE_LIMIT.code, Errors.RATE_LIMIT.message,
        { current: activeCount, max: config.MAX_ACTIVE_INTENTS_PER_USER });
    }
  ...
  async cancelIntent(intentId: string, userId: string): Promise<void> {
    const intent = await this.redis.getIntent(intentId);
    if (!intent) throw new LimitOrderError('83020', 'Intent not found');
    if (intent.userId !== userId) throw new LimitOrderError('83021', 'Unauthorised');
    ...
    await this.stateMachine.transition(
      intentId, intent.status, IntentStatus.CANCELLED, 'User cancelled'
    );
  }
}
